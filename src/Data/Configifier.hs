...
** first attempt:
    ...
    -- | Using '(>.)' is not very convenient: We need a lot of
    ...
    class HasSelector dict sel v where
        (>.) :: dict -> Proxy sel -> v

    instance (KnownSymbol path) => HasSelector (path :> v) path v where
        (>.) (_ :> v) _ = v

    instance (KnownSymbol path) => HasSelector (path :>: descr :> v) path v where
        (>.) (_ :>: _ :> v) _ = v

    instance (HasSelector dict' path v) => HasSelector (dict :| dict') path v where
        (>.) (_ :| dict') = (>.) dict'

this failed because we would need another instance with exactly the
same head as the last, namely:
...
so what if we use value-level strings instead?  less proxy noise in
the accessing expressions, and we get a little further:

    class HasSelector a v where
        (>.) :: a -> String -> Maybe v

    instance (KnownSymbol path) => HasSelector (path :> v) v where
        (>.) (path :> v) sel = if symbolVal path == sel then Just v else Nothing

    instance (KnownSymbol path, HasSelector o v) => HasSelector (path :> v :| o) v where
        (>.) (path :> v :| o) sel = ((path :> v) >. sel) <|> (o >. sel)

    instance (KnownSymbol path, KnownSymbol descr) => HasSelector (path :>: descr :> v) v where
        (>.) (path :>: _ :> v) = (>.) (path :> v)

    instance (KnownSymbol path, KnownSymbol descr, HasSelector o v) => HasSelector (path :>: descr :> v :| o) v where
        (>.) (path :>: _ :> v :| o) = (>.) (path :> v :| o)

slightly better, but there is another problem: if there are two
`fields` in the same `record` with different types, no instance can be
found.
...
use template haskell to construct a nested record type and a lense
from the operator-constructed type into that.
